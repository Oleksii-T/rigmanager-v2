@foreach ($posts as $post)
    <div class="ad-col id_{{$post->id}}">
        <div class="ad-item">
            <div class="ad-img">
                <a href="{{route('posts.show', $post)}}"><img src="{{$post->thumbnail()->url}}" alt=""></a>
                <a href="{{route('posts.add-to-fav', $post)}}" class="catalog-fav add-to-fav {{($currentUser && $currentUser->favPosts->contains($post)) ? 'active' : ''}}">
            ...
            <div class="ad-line">
                ...
                <a href="{{route('search', ['type'=>$post->type])}}" class="ad-tag">{{\App\Models\Post::typeReadable($post->type)}}</a>
            </div>
